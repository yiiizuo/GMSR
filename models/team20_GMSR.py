...
class RFDB(nn.Module):
    ...
    def forward(self, input):
        ...
        # r_c1 = self.c1_rd(r_c1.permute(0, 2, 3, 1)).permute(0, 3, 1, 2)
        r_c1 = self.act(r_c1 + self.c0_rd(input.permute(0, 2, 3, 1)).permute(0, 3, 1, 2))

        distilled_c2 = self.act(self.c2_d(r_c1.permute(0, 2, 3, 1)))
        r_c2 = (self.c2_r(r_c1))
        # r_c2 = self.c2_rd(r_c2.permute(0, 2, 3, 1)).permute(0, 3, 1, 2)
        r_c2 = self.act(r_c2 + r_c1)

        distilled_c3 = self.act(self.c3_d(r_c2.permute(0, 2, 3, 1)))
        r_c3 = (self.c3_r(r_c2))
        # r_c3 = self.c3_rd(r_c3.permute(0, 2, 3, 1)).permute(0, 3, 1, 2)
        r_c3 = self.act(r_c3 + r_c2)

        r_c4 = self.act(self.c4(r_c3))
        # distilled_c3 = self.act(self.c4_d(r_c3.permute(0, 2, 3, 1)))
        # r_c4 = (self.c4_r(r_c3))
        # r_c4 = self.act(r_c3 + r_c2)
...
class GMSR(nn.Module):
    def __init__(self, num_in_ch=3, num_feat=50, num_block=4, num_out_ch=3, upscale=4,
        ...
        if False:
            self.B1 = self.B1.requires_grad_(False)
            self.B2 = self.B2.requires_grad_(False)
            self.B3 = self.B3.requires_grad_(False)
            self.B4 = self.B4.requires_grad_(False)
    ...
    def forward(self, input):
        ...
        return out_fea, out_B1, out_B2, out_B3, out_B4, out_B5, out_lr, output
